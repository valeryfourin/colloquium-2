...
while True:  # програма знаходиться в циклі, який буде виконуватись за умови, що користувач хоче виконати її знову
    ...
    while True:
        try:
            num = float(input('Please input the desired number: ')) # число, з яким будуть порівнюватись елементи масиву А
            for i in A:
                if num not in A:
    ...
    sm_A = [] # числа менші за введене
    mult_A = 1 # результат добутку чисел масиву sm_A

    for i in A:
        if i < num:
            sm_A.append(i) # якщо число задовольняє умову і менше за введене, то воно записується в масив sm_A

    for i in sm_A:
        mult_A *= i # множимо елементи масиву sm_A
    print(f'\nNumbers smaller than {num}: {sm_A}')
    print(f'The multiplication of numbers smaller than {num}: {mult_A}')
